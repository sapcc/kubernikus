...
func Migrate(k *v1.Kluster, client kubernetes.Interface, kubernikus_client kubernikus.Interface) error {
	return defaultRegistry.Migrate(k, client, kubernikus_client)
}
...
func (r *Registry) Migrate(k *v1.Kluster, client kubernetes.Interface, kubernikus_client kubernikus.Interface) error {
	...
	for idx := klusterVersion; idx < r.Latest(); idx++ {
		...
		version := idx + 1
		if kluster, err = migrateKluster(kluster, version, migration, client, kubernikus_client); err != nil {
...
func migrateKluster(kluster *v1.Kluster, version int, migration Migration, client kubernetes.Interface, kubernikus_client kubernikus.Interface) (*v1.Kluster, error) {
	var rawData []byte
	var err error
	...
	if err = migration(rawData, kluster, client); err != nil {
		return nil, err
	}
