...
func (d *showCluster) Handle(params operations.ShowClusterParams, principal *models.Principal) middleware.Responder {
	...
	if err := d.rt.TPRClient.Get().Namespace("kubernikus").Resource(tprv1.KlusterResourcePlural).LabelsSelectorParam(accountSelector(principal)).Name(params.Name).Do().Into(&tprCluster); err != nil {
